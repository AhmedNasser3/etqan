...
class AttendanceController extends Controller
{
        ...
        try {
            ...
            return response()->json([
                ...
                'data'    => $attendance->map(function ($item) {
                    ...
                    if (!$teacherName) $teacherName = 'غير معروف';
                    ...
                    return [
                        ...
                        // وقت الحضور بنظام 12h مع (صباحاً / مساءً)
                        'checkin_time'  => $item->created_at
                            ? AttendanceScheduleService::format12h($item->created_at->format('H:i'))
                            : null,
    ...
    public function quickCheckin(Request $request)
    {
        ...
        if ($existing) {
            return response()->json([
                'success' => true,
                'message' => 'تم تسجيل الحضور اليوم الساعة '
                    . AttendanceScheduleService::format12h($existing->created_at->format('H:i')),
                'data'    => [
                    'status'        => $existing->status,
                    'time'          => AttendanceScheduleService::format12h($existing->created_at->format('H:i')),
            ...
            if ($statusData['status'] === 'late') {
                ...
                if (!$lateReason) {
                    return response()->json([
                        ...
                        // work_start_time يُرسل بنظام 12h
                        'work_start_time'  => AttendanceScheduleService::format12h($schedule->work_start_time),
                    ], 422);
                }
            }
        }
        ...
        $notes = $statusData['status'] === 'late'
            ...
            : "حضور سريع - " . AttendanceScheduleService::format12h($now->format('H:i'));
        ...
        try {
            ...
            $checkinTime12h = AttendanceScheduleService::format12h($now->format('H:i'));

            return response()->json([
                'success' => true,
                'message' => $statusData['status'] === 'late'
                    ? "تم التسجيل - متأخر {$statusData['delay_minutes']} دقيقة"
                    : "تم تسجيل الحضور بنجاح الساعة {$checkinTime12h}",
                'data'    => [
                    'id'            => $attendance->id,
                    'status'        => $attendance->status,
                    'checkin_time'  => $checkinTime12h,     // 12h
    ...
    public function index(Request $request)
    {
        ...
        $attendanceData = $attendances->getCollection()->map(function ($day) {
            return [
                ...
                'day_name'      => $day->date->locale('ar')->dayName,
                ...
                // created_at بنظام 12h
                'created_at'    => AttendanceScheduleService::format12h($day->created_at->format('H:i'))
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // تحقق من حضور اليوم — GET /v1/attendance/today
    // ─────────────────────────────────────────────────────────────────────────
    public function today()
    {
        ...
        // تحقق إجازة
        $service   = app(AttendanceScheduleService::class);
        $isHoliday = $service->isHoliday($centerId, $teacher->id, $todayDate);

        // جدول العمل
        $schedule        = $service->getScheduleFor($centerId, $teacher->id);
        $workStartTime12 = $schedule
            ? AttendanceScheduleService::format12h($schedule->work_start_time)
            : null;
        ...
        return response()->json([
            ...
            'data'           => $todayAttendance->map(function ($attendance) {
                return [
                    ...
                    // وقت الحضور بنظام 12h
                    'checkin_time'  => $attendance->created_at
                        ? AttendanceScheduleService::format12h($attendance->created_at->format('H:i'))
                        : null,
            ...
            'work_start_time' => $workStartTime12,   // 12h
        ], 200, [], JSON_UNESCAPED_UNICODE);
    }

    // ─────────────────────────────────────────────────────────────────────────
    // إحصائيات الحضور — GET /v1/attendance/stats
    // ─────────────────────────────────────────────────────────────────────────
    public function stats(Request $request)
    {
        ...
        $stats = AttendanceDay::where('teacher_id', $teacher->id)
            ...
            ->selectRaw('
                ...
                ROUND(AVG(CASE WHEN delay_minutes > 0 THEN delay_minutes END), 0) as avg_delay
            ')
            ->first();

        return response()->json([
            'success' => true,
            'data'    => $stats ?: (object) ['total' => 0, 'present' => 0, 'late' => 0, 'absent' => 0, 'avg_delay' => 0],
        ]);
    }

    // ─────────────────────────────────────────────────────────────────────────
    // تسجيل حضور جديد — POST /v1/attendance
    // ─────────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // عرض سجل واحد — GET /v1/attendance/{attendanceDay}
    // ─────────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // تعديل سجل حضور — PUT /v1/attendance/{attendanceDay}
    // ─────────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // حذف سجل حضور — DELETE /v1/attendance/{attendanceDay}
    // ─────────────────────────────────────────────────────────────────────────
    ...
    private function calculateTotalStatus(AttendanceDay $day): string
    {
        ...
        if ($day->status === 'late' || ($day->status === 'present' && $day->delay_minutes > 0)) return 'partial';
        return 'present';
    }
}